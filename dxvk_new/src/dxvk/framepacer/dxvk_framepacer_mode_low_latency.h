...
namespace dxvk {
  ...
  public:
    ...
    void startFrame( uint64_t frameId ) override {
      ...
      int32_t lastFrameStart = static_cast<int32_t>(
        duration_cast<microseconds>(m->start - now).count());
      ...
      int32_t cpuReadyPrediction = static_cast<int32_t>(
    ...
    void finishRender( uint64_t frameId ) override {
      ...
      int32_t numLoop = static_cast<int32_t>(m->gpuReady.size()) - 1;
      if (numLoop <= 1
       || m->gpuSubmit.size() != static_cast<size_t>(numLoop)
       || m->gpuQueueSubmit.size() != static_cast<size_t>(numLoop)) {
      ...
      for (int i=0; i<numLoop; ++i) {
        ...
        int32_t duration = static_cast<int32_t>(
          duration_cast<microseconds>(m->gpuReady[i+1] - _gpuRun).count());
        optimizedGpuTime += duration;
        gpuRun.push_back(optimizedGpuTime);
      }

      int32_t alignment = static_cast<int32_t>(duration_cast<microseconds>(
        m->gpuSubmit[numLoop-1] - m->gpuSubmit[0]).count()) - gpuRun[numLoop-1];

      int32_t offset = 0;
      for (int i=numLoop-2; i>=0; --i) {
        int32_t curSubmit = static_cast<int32_t>(duration_cast<microseconds>(
          m->gpuSubmit[i] - m->gpuSubmit[0]).count());
      ...
      props.cpuUntilGpuSync = offset + static_cast<int32_t>(duration_cast<microseconds>(
        m->gpuSubmit[numLoop-1] - m->start).count());
    ...
    Sleep::TimePoint sleepFor( const Sleep::TimePoint t, int32_t delay ) {
      ...
      int32_t frametime = static_cast<int32_t>(
        std::chrono::duration_cast<microseconds>(t - m_lastStart).count());
