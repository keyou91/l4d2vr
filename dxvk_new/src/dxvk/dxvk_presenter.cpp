...
namespace dxvk {
  ...
  void Presenter::runFrameThread() {
    ...
    while (true) {
      ...
      const FramePacer* framePacer = dynamic_cast<FramePacer*>(frame.tracker.ptr());
      if (frame.result >= 0 && (frame.mode == VK_PRESENT_MODE_FIFO_KHR
       || frame.mode == VK_PRESENT_MODE_FIFO_RELAXED_KHR
       || (framePacer && framePacer->getMode()))) {
