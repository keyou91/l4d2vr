...
namespace
{
    ...
    static const CfgEnglishTextOverride kCfgEnglishTextOverrides[] =
    {
        ...
        { "MagazineInteractionUseButtonGripInput", "Hands / Reload", "Button Grip Reload Input", "Uses Reload/Crouch/Jump/SecondaryAttack button inputs for manual reload grip detection.", "Disable for controllers with finger curl or gesture recognition." },
    ...
    static bool CfgSpecParentEnabled(const CfgOverlayState& s, const CfgOptionSpec& spec) // NOLINT(readability-function-cognitive-complexity)
    {
        ...
        // --- Body / First Person ---
        // Character-specific material hiding is still under test. Keep its
        // option metadata, persistence, and material-panel code intact so the
        // two entries can be restored by flipping this exposure flag.
        static constexpr bool kExposeCharacterMaterialHiding = false;
        if (std::strcmp(key, "ClothingMaterials") == 0)
            return kExposeCharacterMaterialHiding &&
...
NekoEnginePostVRTakeover=true
NekoEnginePostVRCaptureBackBuffer=true
NekoEnginePostVRClearTarget=true
NekoEnginePostVRDisableNekoBloom=false
NekoEnginePostVRUseNativeFullFrameSource=false
NekoEnginePostVRHDRSceneInput=true
NekoEnginePostVRDecodeInputSrgb=true
NekoEnginePostVROutputGammaCorrection=false
NekoEnginePostVROutputGamma=2.2
NekoEnginePostProbeLog=true
NekoEnginePostProbeLogHz=2
